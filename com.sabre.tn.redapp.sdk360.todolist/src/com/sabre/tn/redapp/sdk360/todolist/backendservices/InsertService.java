...
public class InsertService implements IInsertService {
	...
	public FlowExtPointCommand execute(FlowExtPointCommand extPointCommand) {
		...
		try {
			...
			list = connector.insertMessgae(m);
			...
			e.printStackTrace();
			addErrorToScreen(extPointCommand,e.getMessage());
		}catch (Exception e) {
	...
	private void addErrorToScreen(FlowExtPointCommand extPointCommand,String error_msg) {
		
		FlowExtPointError extpointError = createMajorError(error_msg); 
		
		addError(extPointCommand, extpointError);
	}
